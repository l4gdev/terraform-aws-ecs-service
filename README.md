...
| <a name="input_cron"></a> [cron](#input\_cron) | Allows to set cron jobs using aws event bridge please check examples | <pre>object({<br>    settings = list(object({<br>      name                = string<br>      args                = string<br>      schedule_expression = string<br>      desired_count       = optional(number, 1)<br>    })),<br>    execution_script = list(string)<br>  })</pre> | `null` | no |
