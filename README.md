...
| <a name="input_application_config"></a> [application\_config](#input\_application\_config) | n/a | <pre>object({<br>    name                   = string,<br>    environment            = string,<br>    cpu                    = number,<br>    memory                 = number,<br>    image                  = string,<br>    port                   = optional(number)<br>    environments_variables = any<br>  })</pre> | n/a | yes |
...
| <a name="input_fargate_datadog_sidecar_parameters"></a> [fargate\_datadog\_sidecar\_parameters](#input\_fargate\_datadog\_sidecar\_parameters) | n/a | <pre>object({<br>    image   = string<br>    dd_site = string<br>  })</pre> | <pre>{<br>  "dd_site": "datadoghq.eu",<br>  "image": "public.ecr.aws/datadog/agent:latest",<br>  "key": ""<br>}</pre> | no |
