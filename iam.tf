resource "aws_iam_role" "ecs-execution" {
  name = lower("${local.tags.Service}-${substr(md5(var.ecs_settings.ecs_launch_type), 0, 20)}-ecs-task-execution-role")
...
resource "aws_iam_role" "service" {
  name = lower("${local.tags.Service}-${substr(md5(var.ecs_settings.ecs_launch_type), 0, 20)}-ecs-task-service-role")
  assume_role_policy = jsonencode({
    Version : "2012-10-17",
    Statement : [
      {
