resource "aws_iam_role" "ecs_task_execution_role" {
  name_prefix = lower("${local.tags.Service}-${substr(md5("${var.application_config.environment}-${var.application_config.name}"), 0, 2)}-ecs-task-execution-role")
...
resource "aws_iam_role" "service_role" {
  name_prefix = lower("${local.tags.Service}-${substr(md5("${var.application_config.environment}-${var.application_config.name}"), 0, 2)}-ecs-task-service-role")
  assume_role_policy = jsonencode({
    Version : "2012-10-17",
    Statement : [
      {
