...
resource "aws_cloudwatch_log_group" "task_log_group" {
  ...
  name_prefix       = "/ecs/${lower(var.ecs_settings.run_type)}/${var.application_config.environment}-${var.application_config.name}"
...
resource "aws_cloudwatch_log_group" "task_log_group_webserver" {
  ...
  name_prefix       = "/ecs/${lower(var.ecs_settings.run_type)}/${var.application_config.environment}-${var.application_config.name}-${var.web_server.name}"
  retention_in_days = var.cloudwatch_logs.retention_in_days
  tags              = local.tags
}
